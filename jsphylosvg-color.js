// Changelog
//
// color-v0.0.1 (mooreryan): color labels and branches based on xml
//
...
Smits.PhyloCanvas.Render.SVG.prototype = {
...
};Smits.PhyloCanvas.Render.Phylogram = function(){ // drawing the phylogram
    ...
    var svg,
        ...
        recursiveCalculateNodePositions = function (node, positionX){
            console.log(" ");
            // console.log("recursiveCalculateNodePositions for node: " + node.name);
            if(node.len && firstBranch == false && node.children.length == 0){
                absoluteY = Smits.Common.roundFloat(absoluteY + scaleY, 4);
            }

            if(node.children.length > 0){ // a horiz line that connects to multiple children branches
                ...
                // Should this be rounded or are half pixels fine?
                var verticalmid = (verticalY1 + verticalY2) / 2;

                // check the colors of the children
                ...
                console.log("are all the children the same color? " + all_children_same_color);
                if (all_children_same_color == 0) {
                    console.log("node: " + node.name + " children are NOT same color");
                    Smits.PhyloCanvas.Render.Style.line.stroke = "black";
                } else {
                    console.log("node: " + node.name + " children are same color"); //ryan
                ...
                if (node.children.length == 2) {
                    ...
                    // child 0 is at the top
                    r = node.children[0].red
                    g = node.children[0].green
                    b = node.children[0].blue
                    Smits.PhyloCanvas.Render.Style.line.stroke = ["rgb(", r, ",", g, ",", b, ")"].join("");
                    svg.draw( // draw top half of vertical
                        new Smits.PhyloCanvas.Render.Path(
                            [
                                "M", positionX + 0.0001, verticalY1,
                                "L", positionX, verticalY1,
                                "L", positionX, verticalmid,
                                // "L", positionX + 0.0001, verticalmid
                            ],
                            { attr : Smits.PhyloCanvas.Render.Style.line }
                        )
                    );

                    // child 1 is at the top
                    r = node.children[1].red
                    g = node.children[1].green
                    b = node.children[1].blue
                    Smits.PhyloCanvas.Render.Style.line.stroke = ["rgb(", r, ",", g, ",", b, ")"].join("");
                    svg.draw( // draw bottom half of vertical
                        new Smits.PhyloCanvas.Render.Path(
                            [
                                // "M", positionX + 0.0001, verticalmid,
                                // "L", positionX, verticalmid,
                                "M", positionX, verticalmid,
                    ...
                    console.log("pie");
                    // draw the whole line at once since it is all black
                    svg.draw( // draw entire line
            ...
            } else { // a line that connects to a single child
                // label
                x1 = positionX;
